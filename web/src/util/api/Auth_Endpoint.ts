...
export async function getOIDCConfig() {
    return fetch(`${reduxStore.getState().auth.resourceServerURL}/.well-known/openid-configuration`)
        .then(r => {
            if(!r.ok) throw new Error(`failed to get configuration from api`)
            return r.json()
        })
}
...
export async function login(code: string){
    ...
    return getOIDCConfig()
        .then(oidcConfig => {
            ...
            .then(r => {
                if(!r.ok) throw new Error(`verifying auth code failed with status ${r.status}`)
                return r.json()
            })
        })
        .then(oidcToken => reduxStore.dispatch(setAuthDetails(oidcToken)))
        .then(() => updateUserInfo())
        .catch(e => console.error(e))
}
...
export async function refresh(){
    ...
    return getOIDCConfig()
        .then(oidcConfig => {
            ...
            .then(r => {
                if(!r.ok) throw new Error(`refreshing the jwt failed with status ${r.status}`)
                return r.json()
            })
        })
        .then(oidcToken => reduxStore.dispatch(setAuthDetails(oidcToken)))
...
export async function logout(){
    ...
    return getOIDCConfig()
        .then(oidcConfig => {
            ...
            .then(() => reduxStore.dispatch(setAuthDetails(undefined)))
        }).catch(e => console.error(e))
}
...
export async function updateUserInfo(){
    ...
    .then(r => {
        if(!r.ok) throw new Error(`failed to sync user info with api`)
        return r.json() as User
    })
    .then(user => reduxStore.dispatch(updateCachedUser(user)))
    .catch(e => console.error(e))
}