...
INSERT INTO users (user_uuid, first_name, last_name, display_name, email, is_system, is_admin, can_login,
...
CREATE FUNCTION UPDATE_GUEST_MEMBER_ON_VISIBILITY_CHANGE()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
...
CREATE TRIGGER update_guest_member_on_visibility_change
    ...
    ON boards
    FOR EACH ROW
EXECUTE PROCEDURE UPDATE_GUEST_MEMBER_ON_VISIBILITY_CHANGE();
...
-- create trigger to set the ticket id to increasing values based on the board
...
DECLARE
    maxID int := 0;
BEGIN
    SELECT MAX(ticket_id) + 1 INTO maxID FROM ticket WHERE board_id = NEW.board_id;
